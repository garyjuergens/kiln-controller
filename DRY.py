...
logger = logging.getLogger('DRY.py')
...
def main():
	...
	if config.startmailsent == 0:
        ...
        elif time_on >= 10 and time_on <15
                config.holdc = 55
                holdhere = config.holdc
               	hold(holdhere,top_tempc)
        elif time_on >= 15 and time_on <20
                config.holdc = 60
                holdhere = config.holdc
                hold(holdhere,top_tempc)
        elif time_on >= 20 and time_on <30
                config.holdc = 70
                holdhere = config.holdc
                hold(holdhere,top_tempc)
        elif time_on >= 30 and time_on <40
                config.holdc = 80 
                holdhere = config.holdc
                hold(holdhere,top_tempc)
        elif time_on >= 40 and time_on <300
                config.holdc = 90
                holdhere = config.holdc
                hold(holdhere,top_tempc)
        else:
		...
		body = "\n".join([
       		"Drying is done, KILN IS OFF and cooling now, FIRE FIRE FIRE!",
       		" "
